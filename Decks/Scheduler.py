...
class Scheduler(object):
    ...
    def answer(self, card, base_time, difficulty):
        self.__calculateEasingFactor(card, difficulty)

        if difficulty == AnswerDifficulty.incorrect:
            card.level = 1

        self.__scheduleCard(card, base_time)